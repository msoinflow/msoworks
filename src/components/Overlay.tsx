  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 md:px-12 lg:px-24 h-16">
        <button
          onClick={handleBack}
          className="text-text-primary font-semibold text-lg tracking-tight hover:text-accent-hover transition-colors duration-200 cursor-pointer"
        >
          MSO
        </button>
        <button
          onClick={handleBack}
          className="text-text-secondary hover:text-accent-hover transition-colors duration-200 text-2xl cursor-pointer"
          aria-label="Close"
        >
          &times;
        </button>
      ...
      <AnimatePresence mode="wait">
        {activeSection === null ? (
            ...
            <div className="flex flex-col items-center gap-6 md:gap-8">
              {menuLinks.map((link) => (
                <button
                  ...
                  className="text-2xl md:text-3xl text-text-secondary hover:text-accent-hover transition-colors duration-300 cursor-pointer"
                >
                  {link.label}
                </button>
              ))}
          ...
          <motion.div
            ...
            className="max-w-5xl mx-auto px-6 md:px-12 lg:px-24 py-12 md:py-20"
          >
...
function StoryContent({
  ...
  const [quizState, setQuizState] = useState<"idle" | "hint" | "revealed">(
    "idle"
  );
              ...
              >
                ...
                <div className="flex gap-4">
                  <motion.button
                    ...
                    className="px-6 py-2 text-sm text-text-secondary border border-border rounded hover:text-accent-hover hover:border-accent-hover transition-colors duration-200 cursor-pointer"
                  >
                    yes
                  </motion.button>
                  <motion.button
            ...
            {quizState === "revealed" && (
              ...
              >
                ...
                <button
                  ...
                  className="text-sm text-accent hover:text-accent-hover transition-colors duration-200 cursor-pointer"
                >
                  → what i do
                </button>
              </motion.div>
      ...
      <div className="mt-12 flex flex-col items-center text-center opacity-40 hover:opacity-100 transition-opacity duration-300">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="px-5 py-2 text-sm text-text-secondary border border-border rounded hover:text-accent-hover hover:border-accent-hover transition-colors duration-200 cursor-pointer"
          >
            what answer?
          </button>
          <button
        ...
        <AnimatePresence mode="wait">
          {!riddleOpen ? (
            <motion.button
              ...
              className="opacity-40 hover:opacity-100 transition-all duration-300 text-sm text-text-tertiary hover:text-accent-hover cursor-pointer"
            >
              stop here, traveler.
            </motion.button>
          ) : (
